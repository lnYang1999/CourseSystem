...
# 会员课程报名
drop table if exists `clubber_course`;
create table `clubber_course` (
                                 `id` char(8) not null default '' comment 'id',
                                 `clubber_id` char(8) not null comment '会员id',
                                 `course_id` char(8) not null comment '课程id',
                                 `at` datetime(3) not null comment '报名时间',
                                 primary key (`id`),
                                 unique key `clubber_course_unique` (`clubber_id`, `course_id`)
